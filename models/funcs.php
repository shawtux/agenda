...
//Check if a rut exists in the DB
...
function fetchUserDetails($email=NULL,$token=NULL, $id=NULL)
{
	...
	$stmt->execute();
	$stmt->bind_result($id, $username, $userlastname, $password, $email, $rut, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn);
	while ($stmt->fetch()){
		$row = array('id' => $id, 'user_name' => $username, 'lastname' => $userlastname, 'password' => $password, 'email' => $email, 'rut'=>$rut, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn);
	}
	$stmt->close();
	return ($row);
}
